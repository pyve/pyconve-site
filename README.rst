...
Sitio web para el PyConVE 2016
...
pyconve_site relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'pyconve_site' environment variables to their Django setting:
...
To run the tests, enter the pyconve_site/tests directory and run the following commands::
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=ve.pycon.org` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/pyconve_site/postgres` by default. Change that if you wan't
...
If you are using `supervisor`, you can use this file as a starting point::
    ...
    [program:pyconve_site]
    command=docker-compose up
    directory=/path/to/pyconve_site
...
Place it in `/etc/supervisor/conf.d/pyconve_site.conf` and run::

    supervisorctl reread
    supervisorctl start pyconve_site
